...
```
$ go run ./rpcgenerate.go  -db mysql -host localhost -user root -password 123456 -package pb -port 3306 -schema test -service_name tester  > test.proto
$ go run ./rpcgenerate.go -db mysql -host localhost -user root  -password 123456 -package pb -port 3306 -schema test -service_name TestService  -file_type csharp_service -table 'person' -ef_namespace "database.efosbasicsys" >test.cs 
